    ...
    <header className="w-full border-b-2 border-[#04102d]/20 bg-white relative z-50">
      <nav className="flex items-center justify-between px-4 sm:px-6 py-4 sm:py-5 max-w-7xl mx-auto w-full relative">
        <Link to="/" className="flex items-center gap-2 cursor-pointer">
          <span
            className="text-[20px] sm:text-[24px] font-black tracking-tighter"
        ...
        <div className="hidden lg:flex items-center gap-8 font-semibold text-[15px] text-[#04102d]/80">
          <Link
            to="/"
            className="hover:text-[#04102d] transition-colors cursor-pointer"
          >
            Home
          </Link>
          <Link
          ...
          {effectivelyLoggedIn ? (
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-3 w-40 bg-white border-2 border-[#04102d] rounded-[16px] shadow-[4px_4px_0_#0bae95] py-2 flex flex-col z-[100] animate-in slide-in-from-top-2 duration-200 overflow-hidden">
                  <Link
                    ...
                    className="text-left px-5 py-2.5 text-[#04102d] font-bold text-[15px] hover:bg-gray-50 transition-colors w-full cursor-pointer"
                  >
                    My Profile
                  </Link>
                  ...
                  <button
                    ...
                    className="text-left px-5 py-2.5 text-[#e13a45] font-bold text-[15px] hover:bg-red-50 transition-colors w-full cursor-pointer"
                  >
                    Logout
                  </button>
                </div>
            ...
            <>
              <button
                ...
                className="px-5 py-2.5 rounded-xl font-bold border-2 border-[#04102d] bg-[#e2eafb] hover:bg-[#d1def8] transition-colors shadow-[2px_2px_0_#04102d] cursor-pointer"
              >
                Sign up
              </button>
              <button
                onClick={() => onOpenAuth("login")}
                className="px-5 py-2.5 rounded-xl bg-[#04102d] text-white hover:bg-gray-800 font-bold transition-colors border-2 border-transparent shadow-[2px_2px_0_#04102d] cursor-pointer"
              >
                Log in
              </button>
            </>
        ...
        <div className="lg:hidden flex items-center gap-3">
          {effectivelyLoggedIn ? (
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white border-2 border-[#04102d] rounded-[16px] shadow-[4px_4px_0_#0bae95] py-2 flex flex-col z-[100] animate-in slide-in-from-top-2 duration-200 overflow-hidden">
                  <Link
                    to="/profile"
                    onClick={() => setIsDropdownOpen(false)}
                    className="text-left px-4 py-2 text-[#04102d] font-bold text-[14px] hover:bg-gray-50 transition-colors w-full cursor-pointer"
                  >
                    My Profile
                  </Link>
                  ...
                  <button
                    ...
                    className="text-left px-4 py-2 text-[#e13a45] font-bold text-[14px] hover:bg-red-50 transition-colors w-full cursor-pointer"
                  >
                    Logout
                  </button>
                </div>
            ...
            <button
              onClick={() => onOpenAuth("login")}
              className="px-4 py-2 rounded-xl bg-[#04102d] text-white hover:bg-gray-800 font-bold sm:font-bold text-sm sm:text-base border-2 border-[#04102d] shadow-[2px_2px_0_#0bae95] transition-all cursor-pointer"
            >
              Log in
            </button>
          )}
          ...
          >
            {isOpen ? (
              <X className="w-5 h-5 text-[#04102d]" />
            ) : (
              <Menu className="w-5 h-5 text-[#04102d]" />
            )}
          </button>
        </div>
      </nav>
      ...
      {isOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-white border-b-4 border-[#04102d] shadow-2xl flex flex-col pt-2 pb-6 px-4 animate-in slide-in-from-top-4 duration-200">
          <Link
            ...
            className="py-4 font-bold text-[#04102d] border-b-2 border-gray-100 flex items-center justify-between"
          >
            <span>Home</span>
          </Link>
          <Link
